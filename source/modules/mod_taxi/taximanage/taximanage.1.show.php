...
if ($saveedit == 1){
	...
	if ($find_chk) {
		...
		$sql = update_db($TableName, array($TableName.'carId='=>$carId), $data);
		//mysql_query($sql);	
...
<script type="text/javascript">
	...
	function fn_goToPage(page,id){
		...
		if (page == 'edit'){
			window.location = 'index.php?p=taxi.taximanage&menu=main_taxi&act=edittaxi&carid='+id+'&garageid='+garageid+''; 
		}
	}
