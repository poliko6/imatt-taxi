<?php
	...
	$aColumns = array(
		'customerMapId',
		'customermap.latitudeCustomer',
		'customermap.longitudeCustomer',
	...
	*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "customermap.latitudeCustomer" ){				
				$row[2] = $aRow['latitudeCustomer'].", ".$aRow['longitudeCustomer'];			
			}
			
				
			if ( $aColumns[$i] == "timeServer" ){				
				$row[3] = Thai_date($aRow['timeServer']);	
			}
			
			if ( $aColumns[$i] == "customerMapId" ){
